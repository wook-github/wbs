...
//			});
		...
		keyEvent : function() {
			...
			$(".isDouble").keypress(function(event){
				...
				event.target.value = wook.wbs.common.doublePattern(v, this);
			}).blur(function(event){
				event = event || window.event;
				var v = event.target.value;
				v = v.replace(/[^0-9^.]/gi, ""); // 숫자와  소수점(.)
			...
			$(".isYmd").on('blur', function(event) {
				...
				if(month > 12) {
					wook.wbs.common.messageBox(null, "날짜 형식 오류", "날짜를 다시 입력해주세요.", null);
					this.value = "";
				} else {
			...
			$(".isTime").keyup(function(event){
			    ...
			    if(replaceTime.length >= 4 && replaceTime.length < 5) {
			        ...
			        if(hours + minute > 2400) {
			        	wook.wbs.common.messageBox(null, "시간 입력", "시간은 24시를 넘길 수 없습니다.", null);
			            this.value = "24:00";
			            return false;
			        }
			        ...
			        // 분은 60분을 넘길 수 없게 세팅
			        if(minute > 60) {
			        	wook.wbs.common.messageBox(null, "시간 입력", "분은 60분을 넘길 수 없습니다.", null);
			        	this.value = hours + ":00";
			        	return false;
			        }
		...
		messageBox : function (eidtObj, msgTitle, msgContent, callback, closeYn, canselYn) {
			wook.wbs.common.editObj		= eidtObj;
			wook.wbs.common.msgTitle		= msgTitle;
			wook.wbs.common.msgContent	= msgContent;
			wook.wbs.common.callback		= callback;
			wook.wbs.common.canselYn		= canselYn;
		...
		cloneFile : function(obj) {
			var browserType = wook.wbs.common.getBrowserType();
		...
		imageFileSelected : function(obj, targetId, size){
			...
			if (fileExt != "jpg" && fileExt != "jpeg" && fileExt != "gif" && fileExt != "png") {
				wook.wbs.common.cloneFile(obj);
				var messageTxt = "(jpg | jpeg | gif | png) 파일만 업로드 가능합니다.";
				wook.wbs.common.messageBox(null, "파일업로드", messageTxt, null);
				return false;
			} else {
				return true;
			}
		...
		downloadFile : function (fileSn) {
			if(fileSn == null || fileSn == '') {
				wook.wbs.common.messageBox(null, "첨부파일 삭제", "삭제할 파일이 없습니다.", null);
				return false;
			}
			...
			$.fileDownload(url,
					{
						...
						failCallback: function() {
							wook.wbs.common.messageBox(null, "엑셀파일 오류", "엑셀파일 다운로드 중 오류가 발생했습니다.", null);
						}
					});
		},
...
function excuteFormValidate(frmId) {
	...
	if ($frms.length) {
		...
		if ($ipts.length) {
			for (var tmpI = 0; tmpI < $ipts.length; tmpI++) {
				...
				if ($(obj).hasClass("isNum")) {
					sValue = wook.wbs.common.removeComma(sValue);
		...
		if (messageTxt != '') {
			wook.wbs.common.messageBox($(obj), messageTitle, messageTxt, null);
		}
